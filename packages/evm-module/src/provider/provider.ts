...
import { InitializationStep, ProviderReadyPromise } from '../utils/provider-ready-promise';
...
export class EVMProvider extends EventEmitter {
  #providerReadyPromise = new ProviderReadyPromise([InitializationStep.PROVIDER_STATE_LOADED]);
  ...
  constructor({
    ...
    super();
    this.info = info;
    this.coreVersion = coreVersion ?? null;
  ...
  #init = async () => {
    try {
      ...
      if (chainId && chainId !== '0x0' && networkVersion && networkVersion !== 'loading') {
      ...
      this.#providerReadyPromise.check(InitializationStep.PROVIDER_STATE_LOADED);
    } catch (e) {
  ...
  #getEventHandler = (method: string): ((params: unknown) => void) => {
    const handlerMap = {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  #handleBackgroundMessage = ({ method, params }: any) => {
  ...
  send = (payload: any, callback?: any) => {
    ...
    switch (payload.method) {
      ...
      default:
        throw new Error('Sync method not supported. Please provide a callback or use the `request` function.');
    }
  ...
  #unlockStateChanged = ({ accounts, isUnlocked }: UnlockStateChangedEventData) => {
    if (isUnlocked) {
      this._isUnlocked = true;
      this._state.isUnlocked = true;
      this.#accountsChanged(accounts);
