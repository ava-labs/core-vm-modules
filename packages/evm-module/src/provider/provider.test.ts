...
const AutoPairingPostMessageConnection = jest.fn().mockReturnValue(mocks);
...
describe('@evm-module/provider', () => {
  ...
  describe('EIP-1193', () => {
    describe('request', () => {
      ...
      it('always returns JSON RPC-compatible error', async () => {
        ...
        (channelMock.request as jest.Mock).mockRejectedValueOnce(new Error('non RPC error'));
      ...
      it('does not double wraps JSON RPC errors', async () => {
        ...
        (channelMock.request as jest.Mock).mockRejectedValueOnce({
          code: 4902,
          message: 'Unrecognized chain ID "0x3FF82". Try adding the chain using wallet_addEthereumChain first.',
    ...
    describe('events', () => {
      describe(`connect`, () => {
        it('should emit `connect` when chainId first set', async () => {
          (channelMock.request as jest.Mock).mockResolvedValueOnce(channelMockResolvedValue);
          const provider = new EVMProvider({ info: providerInfo });
          (addEventListenerSpy.mock.calls[0]?.[1] as any)({
            detail: {
              provider: {
      ...
      describe('disconnect', () => {
        it('emits disconnect event with error', async () => {
          ...
          expect(disconnectSubscription).toHaveBeenCalledTimes(1);
          expect(disconnectSubscription).toHaveBeenCalledWith(providerErrors.disconnected());
        });
      });
    ...
    describe('legacy', () => {
        ...
        it('throws error if method not supported syncronously', async () => {
          ...
          try {
            ...
            expect(error).toStrictEqual(
              new Error('Sync method not supported. Please provide a callback or use the `request` function.'),
            );
          }
        });
      });
      ...
      describe('close event', () => {
        it('should emit close event with error', async () => {
          ...
          expect(closeSubscription).toHaveBeenCalledWith(providerErrors.disconnected());
        });
      });
  ...
  describe('init', () => {
    it('should call the event listener with the right event name', async () => {
      new EVMProvider({ info: providerInfo });
      (addEventListenerSpy.mock.calls[0]?.[1] as any)({
        detail: {
          provider: {
            subscribeToMessage: jest.fn((callback) => {
      ...
      expect(addEventListenerSpy).toHaveBeenCalledWith(EventNames.CORE_WALLET_ANNOUNCE_PROVIDER, expect.any(Function));
    });
