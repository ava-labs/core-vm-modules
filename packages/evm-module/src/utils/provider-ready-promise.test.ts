import { InitializationStep, ProviderReadyPromise } from './provider-ready-promise';

describe('src/background/providers/utils/ProviderReadyPromise', () => {
  it('calls immediately if all checks are checked', async () => {
    const initializedPromise = new ProviderReadyPromise([InitializationStep.DOMAIN_METADATA_SENT]);
