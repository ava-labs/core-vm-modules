import { AVALANCHE_XP_NETWORK, AVALANCHE_XP_TEST_NETWORK, ChainId } from '@avalabs/core-chains-sdk';
import { NetworkVMType, type Network } from '@avalabs/vm-module-types';

export const getNetworkByChainAlias = (chainAlias: string, network: Network): Network => {
  switch (chainAlias) {
    case 'P':
      return network.isTestnet ? P_CHAIN_TEST : P_CHAIN;
    case 'X':
      return network.isTestnet ? X_CHAIN_TEST : X_CHAIN;
    default:
      return network;
  }
};

export const P_CHAIN = {
  ...
  networkToken: {
    ...
    logoUri: 'https://glacier-api.avax.network/proxy/chain-assets/cb14a1f/chains/43114/token-logo.png',
  },
  explorerUrl: 'https://subnets.avax.network/p-chain',
} as Network;
export const P_CHAIN_TEST = {
